...
## Jetson에서 직접 수정/빌드/복사 (Windows 없이)

Jetson 보드에서만 작업하고 싶다면 아래 순서로 바로 반영할 수 있습니다.
...
- `rebuild_so.sh`는 기본적으로 `.so`를 프로젝트 폴더로 복사합니다.
- 헤더(`ec_sample.h`) 변경이 있는 경우 `cp` 단계까지 함께 수행하는 것을 권장합니다.
